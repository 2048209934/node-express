...
//插入图片
router.post('/inlogo',function(req,res){
...
router.post('/uplogo',function(req,res){
	var id=req.body["id"]
	res.header("Access-Control-Allow-Origin", "*");
pool.query(`update logo set  src="${imgs}" where id=${id}`, function(err, rows, fields) {
		pool.query('SELECT * from logo', function(err, rows, fields) {
		if (err) throw err;
	  	res.send(rows)
	});
	});
